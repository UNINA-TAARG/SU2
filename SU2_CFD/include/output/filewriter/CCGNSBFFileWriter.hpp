...
class CCGNSBFFileWriter final: public CFileWriter{

//unsigned long nPoint;   // Global number of nodes with halos
...
public:
   ...
   */
  CCGNSBFFileWriter(string valFileName, CParallelDataSorter* valDataSorter);

  /*!
   * \brief Destructor
   */
  ~CCGNSBFFileWriter() override;

  /*!
   * \brief Write sorted data to file in tecplot ASCII file format
   */
  void Write_Data_BF(CConfig *config, CGeometry *geometry);
